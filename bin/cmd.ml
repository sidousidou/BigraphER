...
let msg_opt fmt = function
  ...
  | States _ -> fprintf fmt "@[<hov>Export each state@ to@ a file@ in %s.@ \
                  ...
                  (colorise `underline "DIR")
                  (string_of_opt "|" (Graph ""))
  | Steps _ -> fprintf fmt "@[<hov>Set the maximum number of simulation \
                            steps.@ This option@ is@ valid@ only@ for@ \
                            deterministic models.@]"
  | Time _ -> fprintf fmt "@[<hov>Set the maximum simulation time.@ This \
                           option@ is@ valid@ only@ for@ stochastic@ models.@]"
  | Verb -> fprintf fmt "@[<hov>Be more verbose.@ This is@ equivalent to@ \
                         setting@ $BIGVERBOSE@ to@ a@ non-empty@ value.@]"
