     ...
     (if (mask land 0b1000) > 0
      then
        (printf "%s\n\n%!" (colorise `bold "Test for BRS");
         (try
            let (ts, stats) = Brs.bfs s reacts 1000 50 v_flag
            in
      ...
      then
        (printf "%s\n\n%!" (colorise `bold "Test for BRS simulation");
         (try
            let (_, stats) = Brs.sim s reacts 1000 50 v_flag
            in printf "%s\n%!" (Brs.string_of_stats stats)
          with
          | e ->
              (printf "%s" (colorise `red "Error: ");
     ...
     (if (mask land 0b0010) > 0
      then
        (printf "%s\n\n%!" (colorise `bold "Test for SBRS");
         (try
            let (ctmc, stats) = Sbrs.bfs s sreacts 1000 50 v_flag
            in
      ...
      then
        (printf "%s\n\n%!" (colorise `bold "Test for SBRS simulation");
         (try
            let (_, stats) = Sbrs.sim s sreacts 5000.0 50 v_flag
