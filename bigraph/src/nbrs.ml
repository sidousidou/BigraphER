    ...
    include
      React.Make (S) (AC)
        (struct
          ...
          let make ~name ~lhs ~rhs ?conds:(c = []) (action, reward, w) eta =
            { name; action; reward; rdx = lhs; rct = rhs; eta; w; conds = c }
          ...
          let step_post (l, n) =
            let normalise (l, n) =
              ...
              in
              ( List.map
                  (fun (b, (a, rew, p), r) -> (b, (a, rew, p /. sum), r))
                  l,
                n )
            in
          ...
          (* Pick the first reaction rule with probability > limit,
             normalising its probability by subtracting the probability of
             the previous rule. *)
