...
let decomp ~target:t ~pattern:p iso =
  ...
  let v_c =
    IntSet.diff
      (IntSet.fold
         (fun i acc -> Sparse.prn trans_t_nn i |> IntSet.union acc)
         v_p' IntSet.empty)
      v_p'
  in
  (* all the other nodes *)
...
let match_list ~target:t ~pattern:p ~n_t ~n_p =
  ...
  let clauses, clauses_exc, cols =
    Sparse.fold
      (fun i j (acc, exc, acc_c) ->
        let a, b =
          (safe @@ Nodes.get_ctrl i n_p, safe @@ Nodes.get_ctrl j n_p)
        in
        let t_edges =
          List.filter
            (fun (i', j') ->
        ...
        | t_edges -> (
            let new_c =
              List.fold_left (fun acc (i', j') -> i' :: j' :: acc) [] t_edges
            in
            match
              List.map
                (fun (i', j') -> (Cnf.M_lit (i, i'), Cnf.M_lit (j, j')))
                t_edges
              |> Cnf.tseitin
            with
            | Cnf.Conj clauses -> (acc, clauses @ exc, new_c @ acc_c)
            | Cnf.Enc (clause, pairs) ->
                ((clause, pairs) :: acc, exc, new_c @ acc_c) ))
      p.nn ([], [], [])
  in
  (clauses, clauses_exc, IntSet.of_list cols)

(* matched columns *)

(* leaves (orphans) in p are matched to leaves (orphans) in t. C5: ij0 or ij1
   or ..... *)
let match_leaves ~target:t ~pattern:p ~n_t ~n_p =
  let l_p = leaves p and l_t = leaves t in
  let clauses, c =
    IntSet.fold
      (fun i (acc, acc_c) ->
        ...
        if IntSet.is_empty compat_t then raise_notrace NOT_TOTAL
          ...
          ( IntSet.fold
              (fun j acc -> Cnf.P_var (Cnf.M_lit (i, j)) :: acc)
              compat_t []
            :: acc,
...
let match_orphans ~target:t ~pattern:p ~n_t ~n_p =
  let o_p = orphans p and o_t = orphans t in
  let clauses, c =
    IntSet.fold
      (fun i (acc, acc_c) ->
        let c = safe @@ Nodes.get_ctrl i n_p in
        let compat_t = IntSet.inter (Nodes.find_all c n_t) o_t in
        if IntSet.is_empty compat_t then raise_notrace NOT_TOTAL
        else
          ( IntSet.fold
              (fun j acc -> Cnf.P_var (Cnf.M_lit (i, j)) :: acc)
...
let check_edges t p v_p' iso =
  ...
  |> List.for_all (fun (u, v) ->
         Sparse.mem p.nn (safe @@ Iso.apply iso' u) (safe @@ Iso.apply iso' v))
      ...
      (fun i j (acc, exc, acc_c) ->
        ...
        | t_edges -> (
            match
              ...
              |> Cnf.tseitin
            with
            | Cnf.Enc (clause, pairs) -> ((clause, pairs) :: acc, exc, acc_c)
            | Cnf.Conj clauses -> (acc, clauses @ exc, acc_c) ))
      p.nn ([], [], [])
  in
  (clauses, clauses_exc, IntSet.of_list cols)
...
let match_region_nodes a b n_a n_b =
  Sparse.fold
    (fun r i (acc, acc_c) ->
      ...
      let children =
        IntSet.filter
          (fun i -> Ctrl.equal c (safe @@ Nodes.get_ctrl i n_b))
          (Sparse.chl b.rn r)
      in
      ( IntSet.fold
          (fun j acc -> Cnf.P_var (Cnf.M_lit (i, j)) :: acc)
