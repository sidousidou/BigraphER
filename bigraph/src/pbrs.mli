...
(** Return the total number of probabilistic reaction rules in a list of
    priority classes. *)
...
val sim :
  ...
  iter_f:(int -> Big.t -> unit) ->
  graph * Stats.t
(** Simulate the PBRS specified by initial state [s0] and priority classes
    ...
    @raise Pbrs.LIMIT when the simulation time exceeds the maximum simulation
    steps. *)
