...
(*
 * Processes the content of [file], adds the variables and clauses to Maple
 * and returns a mapping between names and Maple variables.
 *)
    ...
    let clause =
      List.map
        (fun (sign, name) ->
          ...
          if sign then Maple.pos_lit var else Maple.neg_lit var)
        lits
    in
