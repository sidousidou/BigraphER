...
type t =
  | C of String.t * param list * int (* name, parameters and arity *)
...
let long_name = function
  | C (c, [], _) -> c
  | C (c, ps, _) -> c ^ "("
                     ^ (List.map string_of_param ps
                        |> String.concat ",")
                     ^ ")"

let err =
  "Not a valid string representation of a control"
...
let parse_name s a =
  ...
  | n :: args :: [] ->
    let ps =
      ...
      |> List.map (fun p ->
          try I (int_of_string p) with
          | Failure _ -> try F (float_of_string p) with
            | Failure _ -> S p) in
    C (n, ps, a)
  | _ -> invalid_arg err
...
let to_string c =
  (long_name c) ^ ":" ^ (string_of_int (arity c))
