...
module RT = struct
  ...
  let parse ~name ~lhs ~rhs ?conds:(c = []) p eta =
    { name; rdx = lhs; rct = rhs; eta; p; conds = c }
  ...
  let step b rules =
    RrType.gen_step b rules merge_occ ~lhs ~rhs ~label:l ~map ~conds |> norm
        ...
        and cumulative =
          List.fold_left
            (fun (out, cum_p) (b, p, r) ->
              ...
              ((b, cum_p', r) :: out, cum_p'))
            ([], 0.0)
        and pick = List.find (fun (_, p, _) -> p > Random.float 1.0) in
        ss_sort ss |> cumulative |> fst |> List.rev |> pick |> fun x ->
        (Some x, m)
end
