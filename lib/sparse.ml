...
let compare a b =
  ...
  | 0 -> (match a.c - b.c with
	  | 0 -> ( match compare a.r_major b.r_major with
...
let to_string m =
  ...
  Hashtbl.iter (fun i j -> buff.(i).(j) <- "1")
	       m.r_major;
  buff
...
let row n =
  ...
  for i = n - 1 downto 0 do
    (* 0 -> 0,...,n-1 *)
    Hashtbl.add m.r_major 0 i; (* Hash collisions *)
    (* 0,...,n-1 -> 0 *)
...
let col n =
  ...
  for i = n - 1 downto 0 do
    ...
    (* 0 -> 0,...,n-1 *)
    Hashtbl.add m.c_major 0 i  (* Hash collisions *)
  done;
  m 
...
let mul a b =
  ...
  Hashtbl.iter
    (fun i j ->
     let acc_i = Hashtbl.find_all acc i in
     Hashtbl.find_all b.r_major j
     |> List.iter (fun k ->
		   if List.mem k acc_i
		   then () 
		   else 
