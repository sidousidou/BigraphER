...
let check_states () =
  if defaults.export_states_flag then
    ...
    | (None, Some f) -> (defaults.export_states <- Some (Filename.dirname f); true)
    | (None, None) -> (
      fprintf err_formatter "@[A path must be specified for@ option@ \"-s\"@ \
                               unless it@ used@ with@ option@ \"-t\" @]";
      false)
    | (Some _, None )
    | (Some _, Some _) -> true
  else true
...
let default_cmd =
  ...
  let doc = "An implementation of Bigraphical Reactive System (BRS)
   ...
   Term.info "bigrapher" ~version:"1.9.1" ~doc ~exits:Term.default_exits ~man:[]
