...
(** Encoder for nondeterministic reaction rules. Below is an example output:
    {[{
      "nbrs_lhs": { ... },
                  "nbrs_rhs": { ... },
                  "nbrs_p": 0.476,
...
val occs_to_json : ?minify:bool -> (Bigraph.Big.t * Bigraph.Brs.label) list -> String.t
...
val p_occs_to_json : ?minify:bool -> (Bigraph.Big.t * Bigraph.Pbrs.label) list -> String.t
...
val s_occs_to_json : ?minify:bool -> (Bigraph.Big.t * Bigraph.Sbrs.label) list -> String.t

(** Similar to {!val:Big_json.occs_to_json} but for nondeterministic reactive
    systems. Below is an example output:
    {[
      [
        {
...
val n_occs_to_json : ?minify:bool -> (Bigraph.Big.t * Bigraph.Nbrs.label) list -> String.t
...
(** Encoder for transition systems generated by Stochastic Bigraphical Reactive
    Systems. *)
val ctmc_to_json : ?minify:bool -> Bigraph.Sbrs.graph -> String.t

(** Encoder for transition systems generated by Nondeterministic Bigraphical
    Reactive Systems. *)
...
(** Similar to {!val:Big_json.big_of_json} but for nondeterministic reaction
    rules. *)
val nreact_of_json : ?encoding:Jsonm.encoding -> String.t  -> (Bigraph.Nbrs.react, String.t) result
...
(** [step encoding j] computes the set of states reachable in one rewriting
    ...
    Refer to {!val:Bigraph.Brs.step}, {!val:Bigraph.Pbrs.step},
    {!val:Bigraph.Sbrs.step}, and {!val:Bigraph.Nbrs.step} for more details on
    how reachable states are computed. *)
