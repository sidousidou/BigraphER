...
struct
  ...
  let eval_react name action reward lhs rhs eta conds l scope env env_t p =
    let lhs_v, rhs_v, conds', env_t' = eval_react_aux lhs rhs conds scope env env_t in
    match
    ...
    |> List.fold_left
         (fun (acc, env_t) scope ->
           ...
           try
             ...
             let scope' = extend_scope scope forms exps args_t p in
             let r, env_t'' =
               eval_react id action reward l r eta conds label scope' env env_t' p
             in
             (r :: acc, env_t'')
           with UNIFICATION ->
             raise
  ...
  let store_decs fmt c decs env env_t =
    let rec aux env_t d =
      ...
      | Dreact (React_exp (id, action, reward, lhs, rhs, label, eta, conds, p)) ->
          let r_v, env_t' =
            eval_react id action reward lhs rhs (eval_eta eta) (unpack_cnds conds) label
      ...
      | Dreact
          ...
          upd id
            (React_fun (action, reward, lhs, rhs, eval_eta eta, unpack_cnds conds, label, forms))
            p
  ...
  let ml_of_dec = function
    ...
    | Dreact
        (React_fun_exp (id, action, reward, params, lhs, rhs, l, eta, conds, _)) ->
