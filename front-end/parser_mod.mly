...
let brs_error = mess ^ "Invalid brs\n"
...
%token 	          CTRL BIG REACT SREACT INIT INT FLOAT FUN RATE BRS ENDBRS SBRS
									ENDSBRS RULES
%token            SHARE BY IN
%token            ID IDC IDP
...
dec 
    : CTRL CIDE EQUAL NUM                     
		...
		| RATE IDE EQUAL num_exp   				        { Rate_dec ($2, $4) }
		| FUN RATE FIDE forms RPAR EQUAL num_exp	{ Rate_dec_f ($3, $4, $7) }
    | REACT IDE EQUAL bexp ARR bexp		        { React_dec ($2, $4, $6) }
		| FUN REACT FIDE forms RPAR EQUAL bexp ARR bexp
																			  			{ React_dec_f ($3, $4, $7, $9) }
  	| SREACT IDE EQUAL bexp ARR bexp AT rexp
  	   																				{ Sreact_dec ($2, $4, $6, $8) }
		| FUN SREACT FIDE forms RPAR EQUAL bexp ARR bexp AT rexp  
																							{ Sreact_dec_f 
																									($3, $4, $7, $9, $11) }
...
brs
		: BRS params init rule_set ENDBRS				  { Brs ($2, $3, $4) }
		| SBRS params init rule_set ENDSBRS				{ Sbrs ($2, $3, $4) }
		| error                                   { raise_parse_failure brs_error }
		;
...
init
		: INIT IDE SEMICOLON											{ Init ($2, []) }
		| INIT FIDE args RPAR SEMICOLON						{ Init ($2, $3) }
		;
